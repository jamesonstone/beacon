...
- Bare interactive `beacon` uses a rotating lighthouse sweep and a randomized, non-repeating 150-fact trivia deck during its live scan. Facts change every one to five seconds and fit the terminal width. Redirected output, JSON, and explicit `beacon scan` commands remain animation-free, and cursor state is restored on every exit path.
- Human views treat idle work as inventory rather than queue content. The CLI hides all-idle projects by default behind a compact count and exposes them with `--include-idle`; a targeted `--repo` scan always remains visible. The menu keeps one compact Quiet Projects row that opens a searchable secondary view. Idle base lanes are suppressed when the same project has active work, and Open Top/open-next never fall back to idle inventory, while schema-v2 JSON remains complete and unchanged.
...
4. Make bare `beacon` run the dashboard, offer initialization when config is missing in a TTY, and show a TTY-only lighthouse loader whose 150 original facts appear in randomized non-repeating order at randomized one-to-five-second intervals.
...
8. Render a project-grouped, terminal-width-aware, color-controlled table that prioritizes active work, hides all-idle projects by default, supports `--include-idle`, and keeps JSON ANSI-free and complete.
9. Update the macOS app to decode, group, display, and open schema-v2 project, issue, feedback, and progress evidence, with idle inventory behind a compact count and searchable secondary view.
...
- [x] AC4: Bare Beacon displays the same ordered snapshot as human `scan`, offers init only in a TTY, cycles exactly 150 unique width-safe facts without repetition at one-to-five-second intervals, animates only its interactive live scan, and keeps redirected, JSON, explicit-scan, error, cancellation, and cursor behavior clean.
...
- [x] AC9: Terminal output respects `auto|always|never`, `NO_COLOR`, TTY width, and project grouping; default human output replaces idle inventory with a compact project count, `--include-idle` reveals all-idle projects, targeted repositories remain visible, and JSON retains every lane.
- [x] AC10: The macOS app decodes schema v2, displays the same evidence, opens issue-only lanes, retains last-good results, switches its menu-bar label between a bordered high-contrast non-idle count and a neon-space zero-state glyph, and moves all-idle projects into a searchable secondary view reached through one compact Quiet Projects row.
...
- `make macos-test` passed all 15 Swift tests, including non-idle menu-bar counting, the zero-state path, and quiet-project filtering/search. Xcode emitted non-fatal `linkd.autoShortcut` host-service messages; the suite ended `TEST SUCCEEDED`.
...
- Loader tests verify the trivia deck contains exactly 150 non-empty unique facts, deck exhaustion cannot repeat a selection, randomized delays remain within one to five seconds, timed fact changes occur, and narrow terminals receive an ellipsis-truncated fact.
- Output and Swift state tests verify active views omit idle inventory, redundant idle base lanes disappear when their project has active work, the CLI idle count is project-based, `--include-idle` and targeted scans reveal quiet projects, JSON remains complete, and the menu's searchable Quiet Projects view preserves open targets.
- A live no-refresh scan across the configured 79 projects rendered the active queue first and replaced the former expanded idle section with `32 quiet projects hidden · use --include-idle to show`.
